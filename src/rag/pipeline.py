...
class RAGPipeline:
    ...
    def _expand_query(self, query: str, category: str = None) -> str:
        ...
        focuses on the relevant medical domain.

        Note: the expanded string is used for EVERYTHING downstream in the
        current call path — FAISS encoding, BM25 tokenisation, and the
        cross-encoder reranking pairs (the caller passes one query string
        through). Only the LLM prompt receives the original question.
